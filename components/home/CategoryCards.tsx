...
{/* Category grid */}
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
           {allCategories.map((category, index) => (
             ...
             >
               <div className="h-full w-full text-left p-6 rounded-2xl bg-card/80 backdrop-blur-sm border border-border/50 hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/15 hover:-translate-y-1.5 transition-all duration-250">
                 {/* Icon and color accent */}
                 <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${category.color} flex items-center justify-center text-xl mb-4 transition-transform duration-250 shadow-md`}>
                   {category.icon}
                 </div>
                 ...
                 <div className="mt-auto pt-3 border-t border-border/30 flex items-center justify-between">
                   ...
                   <span className="text-[11px] text-muted-foreground font-medium flex items-center gap-0.5">
                     {category.id === "currency" ? "Popular converter" : "Info only"}
                   </span>
                 </div>
               </div>
             </motion.div>
